...
A = A_bounding(zm,p);                            % evaluate A matrix
bi = bi_bounding(zm,p);                          % evaluate impact b vector (bhat)
i = []; %this will store the values we want to pick out
switch iphase
    case 1
        % flight
...
zplus(1:2:12,1) = zm(1:2:12,1);       % set the positions the same as they were
zplus(2:2:12,1) = xi(1:6,1);          % set the speeds after impact
